    ...
    <ShowOnScroll {...props } showDual >
      <Box sx={theme.containerStyle}>
        <Box sx={{position: "relative"}} >
          ...
          <Box sx={{position: "absolute", color:theme.palette.text.primary, padding: "20px", width: "100%"}}>
            <Box sx={styles.flexContainer}>
              <Typography component="span" sx={styles.fontHeading}>{getDiscountedPrice(data.sell.price, data.sell.discount)}</Typography>
              {data.sell.discount > 0 ?
                <Typography component="span" variant="body2" sx={[styles.discount, {margin: "12px"}]}>{formatter.format(data.sell.price)}</Typography>
                : null }
            </Box>
            {data.sell.discount > 0 ?
              <>
